    ...
    <div role="progressbar" [attr.aria-valuenow]="percent" aria-valuemin="0" aria-valuemax="100" [attr.aria-label]="label || percent + '% del límite'">
      @if (showLabel) {
        <div class="flex justify-between mb-1">
          ...
          <span [class]="'text-xs font-semibold ' + textColorClass">{{ percent }}%</span>
        </div>
      }
      <div [class]="'w-full bg-base-300 rounded-full overflow-hidden ' + trackSizeClass">
        <div
          [class]="fillClass"
          [style.width.%]="percent"
        ></div>
...
export class ProgressBarComponent {
  ...
  get fillClass(): string {
    ...
    const pulse = this.estado === 'critico' && this.animated ? 'animate-pulse' : '';
    return `h-full rounded-full transition-all duration-700 ease-out ${map[this.estado]} ${pulse}`;
  }
